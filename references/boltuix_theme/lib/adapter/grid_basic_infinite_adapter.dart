...
class GridBasicInfiniteAdapter {
  ...
  GridBasicInfiniteAdapter(this.items, onItemClick) {
    ...
    for (var i = 0; i < items!.length; i++) {
      itemsTile.add(
          ItemTile(index: i, object: items![i], onClick: this.onItemClick));
    }
  }
  ...
  void addItems(List<String> items) {
    ...
    for (var i = length; i < this.items!.length; i++) {
      itemsTile.add(ItemTile(
          index: i, object: this.items![i], onClick: this.onItemClick));
    }
  }
  ...
  void setLoading() {
    ...
    itemsTile.add(ItemTile(
        index: length, object: this.items!.last, onClick: this.onItemClick));
  }
    ...
    return NotificationListener(
      ...
      onNotification: (dynamic scrollState) {
        if (scrollState is ScrollEndNotification &&
            scrollState.metrics.pixels == scrollState.metrics.extentBefore) {
