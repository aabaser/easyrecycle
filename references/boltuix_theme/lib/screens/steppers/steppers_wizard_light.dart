...
class SteppersWizardLight extends StatefulWidget {
  SteppersWizardLight();

  @override
  SteppersWizardLightState createState() => new SteppersWizardLightState();
}

class SteppersWizardLightState extends State<SteppersWizardLight> {
  ...
  Widget buildDots(BuildContext context) {
    ...
    for (int i = 0; i < wizardData.length; i++) {
      Widget w = Container(
        ...
        width: 8,
        child: CircleAvatar(
          backgroundColor: page == i ? Colors.pink[400] : MyColors.grey_20,
        ),
      );
      dots.add(w);
    }
